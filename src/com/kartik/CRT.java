...
public class CRT {
    ...
    private static BigInteger E = BigInteger.valueOf(3);        // Public E initially taken as 65537 since its recommended
    private static BigInteger D;                                // Private D
    private static BigInteger dP, dQ;
    private static BigInteger qInverse;
    ...
    private static void encryptDecryptUseKeys(String message) {
        byte[] cipher;
        byte[] plain;
        String cipherTxt;
        String plainTxt;
        ...
        for (int i = 0; i < message.length(); i++) {
            byte[] msg = message.substring(i, i + 1).getBytes();
            cipher = encrypt(msg);
            cipherTxt = new String(cipher);
            ans.append(cipherTxt);
            plain = decrypt(cipher);
            plainTxt = new String(plain);
            ans2.append(plainTxt);
        }
    ...
    private static byte[] decrypt(byte[] message) {
        ...
        BigInteger temp = qInverse.multiply(m1.subtract(m2));
        BigInteger h = temp.mod(P);
